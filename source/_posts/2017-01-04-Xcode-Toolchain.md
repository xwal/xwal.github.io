...
虽然我们来自不同的背景，有不同的观点，塑造我们的经验；虽然我们做事有不同的动机，信念、偏见和意见是我们彼此分离，有一件事我们是在一起的：

**我们都必须使用 Xcode。不管怎样。好的或更坏的。**

至于常见的原因而言，老实说，事情可能更绝望。有几个显著的例外，Xcode似乎每个版本更好。

但是，Xcode 当然不仅仅只是一个应用程序。在 GUI 下是一个应用程序和命令行工具的组合，它们与开发人员的工作流程一样是编辑器的核心。

## Xcode Tools

### xcode-select

讽刺的是，似乎要证明在库比蒂诺没有死亡，每个人与 Xcode 的旅程从一个选择开始。`xcode-select`提供了这个选择，尽管是一个永恒的问题：『蛋糕或死亡？』
...
因为 `xcrun` 的执行是基于当前使用的 Xcode 版本环境（通过 `xcode-select`设置），在系统中存在多个版本的 Xcode 工具链是非常容易的。

在脚本和其他外部工具中使用 `xcrun` 能确保在不同环境中保证一致性的优点。比如，Xcode 附带了代码分发工具 Git。通过调用 `$ xcrun git` 而不是 `$ git`，构建系统可以保证运行正确。
...
| analyze | 从构建根路径(SYMROOT)构建和分析target或者scheme。需要指定 scheme。 |
| archive | 从构建根路径（SYMROOT）打包 scheme。需要指定 scheme。 |
| test | 从构建根路径（SYMROOT）测试 scheme。需要指定 scheme和可选指定目标设备。 |
...
`genstrings` 工具从 指定的 C 或者 Objective-C 源文件生成 `.strings` 文件。使用 `.strings` 文件在不同的 `locale` 本地化应用程序。在苹果的 `Cocoa Core Competencies` 中的 [Internationalization](https://developer.apple.com/library/mac/documentation/general/conceptual/devpedia-cocoacore/- Internationalization.html) 有相关的描述。
...
正如 `genstrings` 的功能针对于源代码，而 `ibtool` 针对于 `XIB` 文件。

```
$ ibtool --generate-strings-file Localizable.strings en.lpoj/Interface.xib
```

虽然本地化是他的主要用例，`ibtool` 拥有对 Interface Builder 文档有效的几个其他功能。它可以使用 `--convert` 标记更改所有对类名的引用。它可以使用 `--upgrade` 标记将文档升级到最新版。它甚至可以启用自动布局，并分别使用 `--enable-auto-layout`、`--update-frames`和`--update-constraints`标记来更新框架和约束。
...
上面的命令将附加到 NSHipster 程序，运行15秒，分析内存分配和泄露，然后将结果写入perf。之后输出结果可以通过 Instruments.app 读取和显示。
...
通过传递 `-w` 标记，`xed` 将等待直到所有打开的窗口关闭。对于脚本化用户交互非常有用，例如提示用户编辑文件并继续一旦完成。

### agvtool

`agvtool` 作用于 Xcode 工程的版本，通过在 Info.plist 文件中读取和写入适当的值。
...
除了上述的 Xcode 工具，还有一些其他可执行文件用 `xcrun` 调用：
...
但是即使自注释代码也可以通过文档来改进，只有少量的努力能够对他人产生显著的效益。

在 Objective-C 中，选择的文档工具是 `appledoc`。使用 `javadoc` 类似的语法，`appledoc` 能够从 .h文件生成 HTML 和 Xcode 兼容的 .docset 文档，看起来几乎完全相同的苹果官方文档。
...
`xctool` 是 `xcodebuild` 的替代品，在 Xcode.app 之下的实用工具。

构建过程的每一步都是整齐地组织和报告，以一种可以理解和视觉上吸引人的方式，和着 ANSI 着色和飞溅的 Unicode 装饰，但 `xctool` 的美丽不是肤浅的：构建进度也能够以格式化报告被其他工具读取：

```
$ xctool -reporter plain:output.txt build
```

* **pretty**: （默认）一个基于文本的报告器，它使用 ANSI 颜色 和 Unicode 符号来处理漂亮的输出。
* **plain**: 类似 pretty，但是没有颜色或 Unicode。
* **phabricator**: 输出一个JSON数组的构建/测试结果，可以输入Phabricator代码审查工具。
* **junit**: 生成具有测试结果的 JUnit/xUnit兼容的XML文件。
* **json-stream**: 构建/测试事件流作为JSON字典，一行（实例输出）。
* **json-compilation-database**: 输出构建事件的JSON编译数据库，可以基于 Clang 工具的工具使用，例如，OCLint。

对 `xcodebuild` 的另一个改进是 `xctool` 将以与 Xcode.app 相同的方式在项目中运行应用程序测试。

仅仅因为这个原因，`xctool` 对 Objective-C 社区中持续集成测试的新兴学科有很大影响。
...
OCLint 是一个静态代码分析器，用于检查 C 代码是否存在常见的问题源，如 空 `if/else/try/catch/finally` 语句，未使用的局部变量和参数，具有高 NCSS（非注释源语句）复杂性，冗余代码，代码气味和其他不良做法。

最好的安装 OCLint 方式是通过 Homebrew Cask:

```
$ brew cask install oclint
```

还记得 xctool 的 `json-compilation-database` 报告器选项吗？使用 `oclint-json-compilation-database` 帮助程序开始 OCLint:
